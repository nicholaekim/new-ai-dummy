...
def simple_metadata(
    ...
    """
    Extract basic metadata from text including dates.
    
    Extracts:
    - Title: First non-empty line
    - Description: First 100 characters of text
    - Date: Extracted using regex_date
    - Extracted date: Raw date string from regex_date
    
    Args:
        text: Text to analyze
        year: Optional expected year for date extraction (not currently used)
        
    Returns:
        Dictionary with 'Title', 'Description', 'Date', and 'extracted_date' keys
    """
    if not text:
        return {
            "Title": "UNKNOWN",
